...
BigCrystalTWI::BigCrystalTWI(uint8_t i2cAddr) : LiquidTWI(i2cAddr) {
...
uint8_t BigCrystalTWI::widthBig(char c) {
  if (!supported(c)) {
...
uint8_t BigCrystalTWI::writeBig(char c, uint8_t col, uint8_t row) {
  if (!supported(c)) {
    return 0; // we don't support characters outside this range
  }
  char ch = toUpperCase(c);
...
uint8_t BigCrystalTWI::printBig(char *str, uint8_t col, uint8_t row) {
  uint8_t width = 0;
  char *c = str;
  while (*c != '\0') {
...
void BigCrystalTWI::getTableCodeAndIndex(char c, uint8_t& tableCode, uint8_t& index) {
...
uint8_t BigCrystalTWI::getWidthFromTableCode(uint8_t tableCode) {
...
bool BigCrystalTWI::supported(char c) {
  return (c >= ' ' && c <= 'Z') || (c >= 'a' && c <= 'z');
}

char BigCrystalTWI::toUpperCase(char c) {
  if (c >= 'a' && c <= 'z') {
    return c &0x5f;
  }
  return  c;
...
void BigCrystalTWI::clearColumn(uint8_t col, uint8_t row) {
  setCursor(col, row);
  write(0x20);
  setCursor(col, row + 1);
  write(0x20);
